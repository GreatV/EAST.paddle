...
def get_score_geo(img, vertices, labels, scale, length):
    ...
    return (
        paddle.to_tensor(data=score_map).transpose(perm=[2, 0, 1]),
        paddle.to_tensor(data=geo_map).transpose(perm=[2, 0, 1]),
        paddle.to_tensor(data=ignored_map).transpose(perm=[2, 0, 1]),
    )
